$(document).ready(function () {
  ...
  $local.tblEnProceso = $local.$tblEnProceso.DataTable({
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          switch (row.estadoSolicitud) {
            case 1:
              a =
                '<span class="badge badge-flat border-info text-info-800">' +
                row.nombreEstadoSolicitud +
                "</span>";
              break;
            default:
              a =
                '<span class="badge badge-flat border-dark text-dark-600">' +
                row.nombreEstadoSolicitud +
                "</span>";
          }
          return a;
      ...
      },
      {
        ...
        render: function (data, type, row) {
          var code =
            '<span style="font-size: 100%; " class="badge badge-light badge-striped badge-striped-left border-left-primary">' +
            row.codigoSolicitud +
            "</span>";
          return code;
        },
      },
      {
        ...
        render: function (data, type, row) {
          var type =
            '<span style="font-size: 100%; " class="badge badge-light badge-striped badge-striped-left border-left-success">' +
            row.nombreConstancia +
            "</span>";
          return type;
        },
      },
      ...
      },
      {
        ...
        render: function (data, type, row) {
          var a =
            row.nombreConstancia +
            " - " +
            row.facultadIniciales +
            " - Cod postulante: " +
            row.codigoPostulante +
            " - DNI: " +
            row.numeroDocumento;
  ...
  $local.$tblEnProceso
    .children("tbody")
    .on("click", "#btnSelect", constanciaSeleccionada);
